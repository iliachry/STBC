...
def simulate_ber_with_common_random_numbers(gamma_opt, gamma_std, snr_db_list, detector='ml', rate=2, num_trials=1000, device=None):
    ...
    channel_seeds = np.random.randint(0, 2**32-1, num_trials)
    noise_seeds = np.random.randint(0, 2**32-1, (len(snr_db_list), num_trials))
    symbol_seeds = np.random.randint(0, 2**32-1, num_trials)
    ...
    for i, snr_db in enumerate(snr_db_list):
        ...
        for trial in range(num_trials):
            ...
            torch.manual_seed(channel_seeds[trial])
            ...
            torch.manual_seed(symbol_seeds[trial])
            if rate == 1:
            ...
            torch.manual_seed(noise_seeds[i, trial])
        ...
        print(f"BER_opt: {ber_opt[i]}")
        ber_std[i] = total_errors_std / (num_trials * len(bits))
        print(f"BER_std: {ber_std[i]}")
